...
def busca_nome_bd(nome_busca):
    ...
    #sql = ("SELECT nomecrianca, DATE_FORMAT(datanasc,'%d-%m-%Y') AS datanascimento,  DATE_FORMAT(dataregistro,'%d-%m-%Y') AS dataDecla,livro,letra,folha,termo FROM nascimentos LIMIT 5")
        ...
        print("************************************************************")
        print("REALIZOU BUSCA DE NASCIMENTO") 
        print("************************************************************")    
        return lista_nomes
def busca_deposito():
...
def cadastro_servico(descricao_servico,data_registro,data_entrega,user_inicio,user_fim,realizado,valor,deposito_previo_iddeposito_previo,pago):
        ...
        print("CRIOU SERVIÇO")
        print("************************************************************")   
def atualizar_servico(cod,data,usuario):
        ...
        sql2 = "UPDATE servico_previo SET user_fim = %s WHERE idservico_previo = %s"
        val2 =(usuario,cod) 
...
def servicos_abertos():
        ...
        total_deposito_previo = None
        mycursor.execute(sql_parte2)
        myresult2 = mycursor.fetchall()
        for i in myresult2:
                total_orcamento = i[0]
                if total_orcamento == None: 
                        total_orcamento = 0
                if valor_total == None:
                        valor_total = 0
...
def servicos_realizado_dia(data):
        ...
        sql ="SELECT nome_solicitante,data_entrega,descricao_servico,valor,user_fim FROM servico_previo join deposito_previo on servico_previo.deposito_previo_iddeposito_previo = deposito_previo.iddeposito_previo where data_entrega = %s AND realizado =1"
        #sql ="SELECT deposito_previo_iddeposito_previo,data_entrega,descricao_servico,valor,user_fim FROM servico_previo where data_entrega = %s AND realizado =1"
        val = (data,)
        mycursor.execute(sql,val)
        myresult = mycursor.fetchall()
...
def pagamento_servico(cod):
        ...
        sql = "UPDATE deposito_previo SET pago = %s WHERE iddeposito_previo = %s"
        val = ("1", cod)
        mycursor.execute(sql, val)
        sql2 ="UPDATE deposito_previo SET data_criacao = %s WHERE iddeposito_previo = %s"
        val2 = (data, cod)
        mycursor.execute(sql2,val2)
        
        sql1 ="UPDATE servico_previo SET pago= 1 WHERE deposito_previo_iddeposito_previo= %s"
        val1 = (cod,)
        mycursor.execute(sql1,val1)
        print("************************************************************")
        print("PAGAMENTO REALIZADO COM SUCESSO --- COD ",cod) 
        print("************************************************************")  
